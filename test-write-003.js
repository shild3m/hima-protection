...
-- This function gets the first service from referral_services.
-- ============================================================
...
BEGIN
  ...
  -- FIX: Get service_id from referral_services junction table (not referrals.service_id which does not exist)
...
fs.writeFileSync(
  path.join(__dirname, 'supabase', 'migrations', '003_fix_security_migration.sql'),
  sql,
  'utf8'
);

console.log('Written successfully:', sql.length, 'chars');
...
console.log('DROP POLICY IF EXISTS:', dropCount);
console.log('CREATE POLICY:', createCount);
console.log('Match:', dropCount === createCount ? 'YES' : 'NO - MISMATCH');
