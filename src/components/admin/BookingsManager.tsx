...
const STATUS_META: Record<string, { icon: React.ReactNode; iconBg: string; iconColor: string }> = {
  ...
  cancelled: { icon: <FaBan />, iconBg: 'bg-[#F6F6F6]', iconColor: 'text-[#9CA3AF]' },
}
...
const statusBadge = (status: string) => {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold border ${config.bg} ${config.color}`}>
 ...
 <p className="text-[#9CA3AF] text-xs md:text-sm mt-0.5">عرض وإدارة حجوزات العملاء</p>
 </div>
 </div>
 {canCreate && (
 <button
 ...
 <div className={`text-[11px] font-bold mt-2 ${active ? config.color : 'text-[#9CA3AF]'}`}>{config.label}</div>
 </div>
 ...
 <FaSearch className="absolute right-4 top-1/2 -translate-y-1/2 text-[#9CA3AF] text-sm" />
 <input
 type="text"
 placeholder="بحث بالاسم أو الجوال أو الماركة أو رقم اللوحة..."
 value={search}
 onChange={(e) => setSearch(e.target.value)}
 className="w-full bg-white border border-[#E7E8EA] shadow-sm shadow-black/[0.02] rounded-2xl pr-11 pl-10 py-3 text-sm text-[#111214] focus:outline-none focus:border-[#DC2626] focus:ring-4 focus:ring-red-500/10 transition-all placeholder:text-[#9CA3AF]"
 />
 {search && (
 <button onClick={() => setSearch('')} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#9CA3AF] hover:text-[#111214] transition">
 <FaTimes className="text-xs" />
 </button>
 )}
 </div>
 ...
 <span className="text-sm text-[#9CA3AF] font-medium">جاري تحميل الحجوزات...</span>
 </div>
 </div>
 ) : bookings.length === 0 ? (
 ...
 <p className="text-xs text-[#9CA3AF] mt-1">لم يتم العثور على حجوزات تطابق البحث الحالي</p>
 </div>
 ) : (
 <>
 ...
 <div className="flex items-center gap-x-3 gap-y-1 mt-2 text-xs text-[#62666D] flex-wrap">
 {booking.customer?.phone && (
 <span className="flex items-center gap-1.5 font-medium" dir="ltr">
 <FaPhone className="text-[#059669]/70 text-[10px]" />
 {booking.customer.phone}
 </span>
 )}
 {booking.vehicle && (
 <span className="flex items-center gap-1.5 font-medium">
 <FaCar className="text-[#9CA3AF] text-[10px]" />
 {booking.vehicle.make} {booking.vehicle.model}
 </span>
 )}
 {booking.service && (
 <span className="flex items-center gap-1.5 font-medium">
 <FaWrench className="text-[#9CA3AF] text-[10px]" />
 {booking.service.name}
 </span>
 )}
 <span className="flex items-center gap-1.5 font-medium">
 <FaClock className="text-[#9CA3AF] text-[10px]" />
 ...
 <div className="px-3.5 py-2 text-[10px] font-bold text-[#9CA3AF] bg-[#FBFBFA] border-b border-[#E7E8EA] flex items-center gap-1.5">
 ...
 <span className="font-bold text-[#111214]">{pagination.total}</span> نتيجة — صفحة <span className="font-bold text-[#111214]">{pagination.page}</span> من {pagination.totalPages}
 </span>
 ...
 <p className="text-xs text-[#9CA3AF] mt-0.5">{viewingBooking.customer?.full_name || 'عميل'}</p>
 </div>
 </div>
 ...
 <button onClick={() => setViewingBooking(null)} className="text-[#9CA3AF] hover:text-[#111214] hover:bg-[#F7F7F5] p-2.5 rounded-xl transition-all">
 ...
 <span className="text-xs font-bold text-[#9CA3AF] flex items-center gap-1.5">
 ...
 <div className="px-4 py-3 space-y-2.5">
 <div className="flex justify-between text-sm">
 <span className="text-[#9CA3AF]">المركبة:</span>
 ...
 <div className="flex justify-between text-sm">
 <span className="text-[#9CA3AF]">التاريخ المفضل:</span>
 ...
 <span className="text-[#9CA3AF]">ملاحظات العميل:</span>
 ...
 <span className="text-[#9CA3AF] font-medium">{new Date(h.created_at).toLocaleDateString('ar-SA')}</span>
 <span className="text-[#9CA3AF]">→</span>
 {statusBadge(h.new_status)}
 {h.notes && <span className="text-[#9CA3AF]">({h.notes})</span>}
 </div>
 ))}
 </div>
 </div>
 ...
 <button onClick={() => setShowCreateForm(false)} className="text-[#9CA3AF] hover:text-[#111214] hover:bg-[#F7F7F5] p-2.5 rounded-xl transition-all">
 <FaTimes className="text-sm" />
 </button>
 </div>
 ...
 <p className="text-xs text-[#9CA3AF] mt-0.5">لا يمكن التراجع عن هذا الإجراء</p>
 </div>
 </div>
