...
BEGIN
  ...
  -- 4. Idempotency check via bookings.idempotency_key (UNIQUE column)
  ...
  -- 5. Rate limiting: max 3 per phone per day
  IF (
  ...
  -- 6. Find or create customer (phone is UNIQUE)
  ...
  -- 7. Find or create vehicle (scoped to customer)
  ...
  -- 8. Create booking (status = 'new', lowercase per CHECK constraint)
